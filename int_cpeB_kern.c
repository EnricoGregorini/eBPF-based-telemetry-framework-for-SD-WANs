...
int tc_sink(struct __sk_buff *skb) 
{
    ...
    */
    ...
    if (mdentry_source->sequence_number - *rec_counter > 1){
        ...
        u32 lost_packet_id = mdentry_source->sequence_number - 1;
        packet_lost_event.perf_submit(skb, &lost_packet_id, sizeof(u32));
    } else {
    ...
    u64 owd = 0;  // One Way Delay set to 0 by default (if the packet is not valid, the OWD is not calculated)
    if (is_valid_packet) {
        // Calculate the OWD
