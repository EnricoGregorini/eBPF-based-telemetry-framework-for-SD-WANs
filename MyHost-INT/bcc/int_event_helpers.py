...
file_freq = 10000  # Frequency of saving INT events to a file

local_addr = ni.ifaddresses('gre1')[ni.AF_INET][0]['addr']
bpf_src_file = "int_cpeA_kern.c" if local_addr == "10.0.0.1" else "int_cpeB_kern.c"
owd_filename = "results/SinusoidalDelay/int_BtoA_owd_records.json" if local_addr == "10.0.0.1" else "results/SinusoidalDelay/int_AtoB_owd_records.json"
twd_filename = "results/SinusoidalDelay/int_twd_records.json" 
...
def print_int_sink_event(cpu, data, size):
    ...
    if test == "twd":
        ...
        if len(sink_events) > 0 and len(local_src_events) > 0:
            ...
            if test == "twd" and len(twd_records) % 10 == 0:
